...
var (
	zoneFGColor  = color.RGBA{0xdb, 0xde, 0xe1, 0xff}
	zoneBGColor  = color.RGBA{0x31, 0x33, 0x38, 0xff}
	zoneFGImage  = image.NewUniform(zoneFGColor)
	zoneBGImage  = image.NewUniform(zoneBGColor)
	zoneFont     = goregular.TTF
	zoneFontBold = gobold.TTF
)
...
func (e *ZonePreview) writeNoAlign(field string, value string) {
...
func (e *ZonePreview) writeNoAlignLn(field string, value string) {
	e.writeNoAlign(field, value)
	e.newLine(1)
}

func GenerateZonePreview(zoneIcon int32, lines []string) ([]byte, error) {
	mu.RLock()
	defer mu.RUnlock()

	var newPos fixed.Point26_6

	font, err := truetype.Parse(zoneFont)
	if err != nil {
	...
	fontBold, err := truetype.Parse(zoneFontBold)
	if err != nil {
		return nil, fmt.Errorf("parse fontBold: %w", err)
	}

	rgba := image.NewRGBA(image.Rect(0, 0, 700, 600))
	draw.Draw(rgba, rgba.Bounds(), zoneBGImage, image.Pt(0, 0), draw.Src)
	...
	for i, line := range lines {
		if i == 0 {
			line = strings.ReplaceAll(line, "Zone Info for Effect: ", "")
			iconImg := store.ZoneIcon(int32(zoneIcon))
			titleOffset := 10
			// draw a 40x41 image
			if iconImg != nil {
	...
	if e.maxWidth != 700 || e.maxHeight != 600 {
		...
		draw.Draw(rgba2, rgba2.Bounds(), zoneBGImage, image.Pt(0, 0), draw.Src)
...
func (e *ZonePreview) newLine(lineCount int) {
	e.lineCurrent += lineCount
	e.shiftLn(lineCount)
}

func (e *ZonePreview) shiftLn(lineCount int) {
...
func (e *ZonePreview) setCursor(x int, y int) {
