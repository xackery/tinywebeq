...
var (
	spellFGColor  = color.RGBA{0x31, 0x33, 0x38, 0xff}
	spellBGColor  = color.RGBA{0xdb, 0xde, 0xe1, 0xff}
...
func GenerateSpellPreview(spellIcon int32, lines []string) ([]byte, error) {
	mu.RLock()
	defer mu.RUnlock()
	...
	for i, line := range lines {
		if i == 0 {
			...
			iconImg := store.SpellIcon(spellIcon)
			titleOffset := 10
			// draw a 40x41 image
			if iconImg != nil {
