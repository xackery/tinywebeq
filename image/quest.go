...
var (
	questFGColor  = color.RGBA{0xdb, 0xde, 0xe1, 0xff}
	questBGColor  = color.RGBA{0x31, 0x33, 0x38, 0xff}
	questFGImage  = image.NewUniform(questFGColor)
	questBGImage  = image.NewUniform(questBGColor)
	questFont     = goregular.TTF
	questFontBold = gobold.TTF
)
...
func (e *QuestPreview) writeNoAlign(field string, value string) {
...
func (e *QuestPreview) writeNoAlignLn(field string, value string) {
	e.writeNoAlign(field, value)
	e.newLine(1)
}

func GenerateQuestPreview(questIcon int, lines []string) ([]byte, error) {
	mu.RLock()
	defer mu.RUnlock()

	var newPos fixed.Point26_6

	font, err := truetype.Parse(questFont)
	if err != nil {
	...
	fontBold, err := truetype.Parse(questFontBold)
	if err != nil {
		return nil, fmt.Errorf("parse fontBold: %w", err)
	}

	rgba := image.NewRGBA(image.Rect(0, 0, 700, 600))
	draw.Draw(rgba, rgba.Bounds(), questBGImage, image.Pt(0, 0), draw.Src)
	...
	if e.maxWidth != 700 || e.maxHeight != 600 {
		...
		draw.Draw(rgba2, rgba2.Bounds(), questBGImage, image.Pt(0, 0), draw.Src)
...
func (e *QuestPreview) newLine(lineCount int) {
	e.lineCurrent += lineCount
	e.shiftLn(lineCount)
}

func (e *QuestPreview) shiftLn(lineCount int) {
...
func (e *QuestPreview) setCursor(x int, y int) {
