...
func spellEffect(se *Spell, index int) string {
	...
	case SPA_FOCUS_TARGET_TYPE:
		out += formatLimits(spellEffectName, value, TargetTypeLimitsName(int(base)))
		out += spellRange
	case SPA_FOCUS_WHICH_SPA:
		out += formatLimits(spellEffectName, value, spellEffectNameByID(int(base)))
		out += spellRange
	...
	case SPA_FOCUS_LIMIT_CLASS:
		out += formatExtra(spellEffectName, db.ClassesFromMask(base), spellRange, "")
