...
func calcValue(calc int32, base int32, max int32, tick int32, minLevel int32, level int32) int32 {
	if calc == 0 {
		return base
	}
...
func calcValueRange(calc int32, base int32, max int32, duration int32, minLevel int32, level int32, usePercent bool) string {
...
func calcExtendedRange(calc int32, start int32, finish int32, minLevel int32, maxLevel int32, usePercent bool, useACMod bool) string {
	percent := ""
	if usePercent {
		percent = "%"
	}
	...
	default:
		startVal := start
		if useACMod {
			startVal = int32(float64(start) / float64(10/3))
		}
		finishVal := finish
		if useACMod {
			finishVal = int32(float64(finish) / float64(10/3))
		}
...
func calcMaxSpellLevel(calc int32, base int32, max int32, tick int32, minLevel int32, level int32) int32 {
	if abs(max) < 1 {
		return level
	}
	for maxLevel := int32(1); maxLevel < level; maxLevel++ {
...
func spellDurationCalc(calc int32, level int32) int32 {
	switch calc {
	case 1:
		if level > 3 {
			return level / 2
...
func formatPercent(spellEffectName string, value int32, max int32, scaling bool, hundreds bool, percent string) string {
	out := ""
	tmp := "Increase"
	if max < 0 {
		tmp = "Decrease"
...
func formatRange(spellEffectName string, value int32, spellRange string, extra string) string {
	tmp := "Increase"
	if value < 0 {
		tmp = "Decrease"
	}
...
func formatCount(spellEffectName string, value int32, preposition string, percent string) string {
	if preposition == "" {
		preposition = "by"
	}
	tmp := "Increase"
...
func formatAT(spellEffectName string, value int32, preposition string, percent string) string {
	if preposition == "" {
		preposition = "by"
	}
	return fmt.Sprintf("%s %s %d%s", spellEffectName, preposition, abs(value), percent)
}
func formatBase(spellEffectName string, base int32, max int32, optional string) string {
	if max == 0 {
		if optional != "" {
...
func formatBasePercent(spellEffectName string, base int32) string {
	return fmt.Sprintf("%s (%d%%)", spellEffectName, base)
}

func formatMinMaxBase(spellEffectName string, base int32, spa int32) string {
...
func formatLimits(spellEffectName string, value int32, extra string) string {
	tmp := "allowed"
	if value < 0 {
		tmp = "excluded"
	}
...
func formatMax(spellEffectName string, value int32, max int32) string {
	tmp := "Increase"
	if value < 0 {
		tmp = "Decrease"
	}
...
func formatPenaltyChance(spellEffectName string, value int32, penaltychance string) string {
	if value < 100 {
...
func formatRateMod(spellEffectName string, value int32, base int32) string {
	if base > 0 {
...
func formatRefreshTimer(spellEffectName string, value int32, max int32, skill int32, preposition string) string {
	tmp := "Increase"
	if max < 0 {
		tmp = "Decrease"
	}
	tmpSkill := "All Skills"
	if skill >= 0 {
		tmpSkill = library.SkillName(skill)
	}
	if preposition == "" {
		preposition = "with"
	}
...
func formatResists(spellEffectName string, value int32, base int32) string {
	if value >= 100 {
		return spellEffectName
	}
...
func formatSeconds(spellEffectName string, value int32, tens bool) string {
	if tens {
...
func formatSkillAttack(spellEffectName string, value int32, max int32, base2 int32, skill int32, preposition string) string {
	if preposition == "" {
		preposition = "with"
	}
	tmpSkill := "All Skills"
...
func formatSkills(spellEffectName string, value int32, max int32, skill int32, percent string, preposition string) string {
	if preposition == "" {
		preposition = "with"
	}
	tmpSkill := "All Skills"
...
func formatSpellChance(spellEffectName string, value int32, base int32) string {
	if value > 100 {
...
func formatSpellGroupChance(spellEffectName string, value int32, groupId int32) string {
	if value > 100 {
...
func formatStacking(spellEffectName string, slot int32, value int32, max int32, spa int32, extra string) string {
	tmp := "new"
...
func formatStatsCapRange(spellEffectName string, value int32, stat string, spellRange string) string {
	tmp := "Increase"
	if value < 0 {
		tmp = "Decrease"
	}
